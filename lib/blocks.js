...
/**
 * Find x2000 block headers by hash ...
 */
BlockController.prototype.blockHeaders = function(req, res) {
  var self = this;
  let hash = req.params.blockHash;
  let nbOfBlocks = req.params.nbOfBlocks;

  let cb = function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    req.blockHeaders = info;
    // next();
  };
  let nbOfBlockToFetch = nbOfBlocks ? nbOfBlocks : false;
  self.node.services.bitcoind.getBlockHeaders(hash, cb, nbOfBlockToFetch);
};
